...
char checknamey1[24] = "check_myparametry1.txt";//для проверки 
...
double DofMfunction(double P) {
  //return 730.0;//?????????????????????
  double alo;
  alo=(730.0-700.0)/(25000000.0-101325.0);
  return 710.0+alo*(P-101325.0);// нужно подправить альфа 5e-6
}
...
double DwfMfunction(double P) {
 ...
 alw=(1118.0-1000.0)/(25000000.0-101325.0);
  return 1000.0+alw*(P-101325.0); // нужно подправить альфа 5e-6
}
...
int main(int argc, char *argv[])
{
  ...
  double *mfw_Dwfw,*mfw_Dwfw_t0,*mfw_Dofw,*mfw_Dofw_t0,*Prf_t0ff,*Prfsff_time;
  double *hclff,*hknff;
...
//  u10 = u1 - u0; omg0 = 1.0 / tau0; omg1 = 1.0 / tau1;
  ...
  y1 = (double*)(malloc(sizeof(double)*(nc+1)));

  

  Apkff = (double*)(malloc(sizeof(double)*(nc+1)));
  Bpkff = (double*)(malloc(sizeof(double)*(nc+1)));
  Cpkff = (double*)(malloc(sizeof(double)*(nc+1)));
...
//свои переменные
  ...
  Prf_t0ff=(double*)(malloc(sizeof(double)*(nc+1)));
  Fpkff=(double*)(malloc(sizeof(double)*(nc+1)));
...
fprintf(Fy1,"%20s\n","y1");
  ...
  xx[i] = xa + hx * (i); 
   Prfsff[i]=25E6;// явный
  //Prfsff[i]=Prfsff[i]+y1[i];// явный
 // printf("%20.13le\n",y1[i]);
  Prf_t0ff[i]=25E6;//неявный
  }// initial profile

for (i=200; i<=800; i++) {
Prfsff[i]=Prfsff[i]-10.0;
}
...
for (i=0; i<=nc; i++) 
{
  hknff[i]=0.5*(hclff[i]+hclff[i+1]);
  //printf("%f\n",hknff[i]);  
}


// Time loop:
time=0.01;//1*24*60*60
S_count=0;
epst1=0.001;
epst2=0.0001;
...
for (i=0; i<=nc+1; i++)
{Apkff[i]=0.0;Cpkff[i]=0.0;Bpkff[i]=0.0;Fpkff[i]=0.0;y1[i]=-0.0;
kfhff[i]=0.0;Dwfwdfff[i]=0.0;Dofwdfff[i]=0.0;MwfMff[i]=0.0;MofMff[i]=0.0;Swfsff[i]=0.0;
Swfw_Dwfw_dfff[i]=0.0;Swf1w_Dofw_dfff[i]=0.0;mfw_Dwfw_Pf[i]=0.0;mfw_Dofw_Pf[i]=0.0;
mfw_Dwfw[i]=0.0;mfw_Dwfw_t0[i]=0.0;}
...
for (i=0; i<=nc+1; i++) {
 ...
 ves=d(Prfsff[i],Prf_t0ff[i]);
    
   
                
        kfhff[i]=1.0E-12/hknff[i];//проницаемость/шаг 
        Dwfwdfff[i]=1.0/(ves*DwfMfunction(Prfsff[i])+(1.0-ves)*DwfMfunction(Prf_t0ff[i]));//обратная плотность воды c весами
        if (i!=0)
        
        {
          DwfMff[i]=0.5*(DwfMfunction(Prfsff[i])+DwfMfunction(Prfsff[i-1]));//плотность воды(к-+0.5)
          DofMff[i]=0.5*(DofMfunction(Prfsff[i])+DofMfunction(Prfsff[i-1]));//плотность нефти(к-+0.5)
        }
        
        else{DwfMff[0]=DwfMff[1];DofMff[0]=DofMff[1];}
        
        
        MwfMff[i]=0.67E-3;// вязкость воды
        Dofwdfff[i]=1.0/(ves*DofMfunction(Prfsff[i])+(1.0-ves)*DofMfunction(Prf_t0ff[i]));//обратная плотность нефти с весами
        ...
        mfw_Dwfw_Pf[i]=hknff[i]*0.01*DwfM_Pfunction(Prfsff[i]);//шаг*(пористость)'* плотность воды+пористость*(плотность воды)' (неявный слой)
        mfw_Dofw_Pf[i]=hknff[i]*0.01*DofM_Pfunction(Prfsff[i]);//шаг*(пористость)'*(плотность нефти)+пористость*(плотность нефти)' (неявный слой)
        
        mfw_Dwfw[i]=0.01*hknff[i]*DwfMfunction(Prfsff[i]);//1.1e-4//_пористость*плотность воды (неявный слой)!!!!добавить шаг
        mfw_Dwfw_t0[i]=0.01*hknff[i]*DwfMfunction(Prf_t0ff[i]);//_пористость*плотность воды (явный слой)
        mfw_Dofw[i]=0.01*hknff[i]*DofMfunction(Prfsff[i]);//_пористость*плотность нефти (неявный слой)
        mfw_Dofw_t0[i]=0.01*hknff[i]*DofMfunction(Prf_t0ff[i]);//_пористость*плотность нефти (явный слой)

        //fprintf(Fc,"%20.13le %20.13le %20.13le %20.13le %20.13le %20.13le\n",kfhff[i],Dwfwdfff[i],DwfMff[i],MwfMff[i],Swfsff[i],Dofwdfff[i]);
        
  }
  DwfMff[0]=DwfMff[1];DwfMff[nc+1]=DwfMff[nc];
  DofMff[0]=DofMff[1];DofMff[nc+1]=DofMff[nc];
...
//
  ...
  if (i!=nc) // !no_rigtt_bound
	 ...
	 Fpkff[i]=Fpkff[i]+\
      ...
      Dofwdfff[i]*(DofMff[i+1]/MofMff[i+1])*\
      Kroff(Swfs_up_loc)\
      )*((Prfsff[i+1]-Prfsff[i]));// !(-!darsy)=- исправь
...
//if(i<=100)
{
  ...
  Apkff[i]=tau*kfhff[i]*\
   (Dwfwdfff[i]*(DwfMff[i]/MwfMff[i])*Krwff(Swfs_up_loc)+\
    Dofwdfff[i]*(DofMff[i]/MofMff[i])*Kroff(Swfs_up_loc));
*/
    //fprintf(Fc,"%20.13le %20.13le %20.13le \n",Apkff[i],DofMff[i],MofMff[i]);
    fprintf(Fabc,"%20.20le %20.20le %20.20le %20.20le %20.20le\n",Apkff[i],Cpkff[i],Bpkff[i],Fpkff[i],y1[i]);
    // нужно спросить 
    //fprintf(Fy1,"%20.25le %20.25le %20.25le\n",y1[i],epst1*Prf_t0ff[i]+epst2,(-Apkff[i]*y1[i-1]+Cpkff[i]*y1[i]-Bpkff[i]*y1[i+1])-Fpkff[i]);
}


//Prfsff[i]=Prf_t0ff[i];
//Prf_t0ff[i]=Prf_t0ff[i]+y1[i];
Prf_t0ff[i]=Prfsff[i];
Prfsff[i]=Prfsff[i]+y1[i];

fprintf(Fy1,"%20.13le \n",y1[i]);
}
Prfsff_time[ntv]=Prf_t0ff[1];
...
printf("%20.8le\n",Prfsff_time[ntv]);
fprintf(Fabc,"%20.25le\n",tau);
ntv++; tv += tau;


/*
 for (i=0; i<=nc; i++){
    if (y1[i]<(epst1*Prf_t0ff[i]+epst2))
    {
      Prfsff[i]=Prf_t0ff[i];
      Prf_t0ff[i]=Prf_t0ff[i]+y1[i];
      ntv++; tv += tau;
    }
    else{Prfsff[i]=Prf_t0ff[i];
      Prf_t0ff[i]=Prf_t0ff[i]+y1[i];
      ntv++; tv += tau;
    S_count=S_count+1;
    if (S_count>6)
    ...
    */
    ...
    //(y1<epst1*Prf_t0ff[i]+epst2)&&(S_count<6);
  ...
  sprintf(sname,"%s_%02d.txt",vname,np);
  OutFun1DP(sname,np,mp,nc,xx,Prf_t0ff);
